...
class NetworkMonitor extends events_1.EventEmitter {
     ...
     */
    async stopMonitoring(containerId) {
     ...
     */
    async setupProxyAccess(containerId, proxyHost, proxyPort, allowedDomains) {
        try {
            const container = this.docker.getContainer(containerId);
     ...
     */
    logNetworkActivity(containerId, activity) {
    ...
    matchesPattern(activity, pattern) {
        ...
        if (pattern.pattern.ipRanges) {
            const matchesIpRange = pattern.pattern.ipRanges.some(range => this.isIpInRange(activity.destinationIp, range));
            if (!matchesIpRange) {
                return false;
            }
        }
     ...
     */
    checkAlertThresholds(containerId) {
        const logs = this.networkLogs.get(containerId) || [];
        const now = new Date();
     ...
     */
    async setupNetworkNamespace(containerId) {
        try {
            const container = this.docker.getContainer(containerId);
     ...
     */
    async collectNetworkActivity(containerId) {
        try {
            const container = this.docker.getContainer(containerId);
     ...
     */
    parseNetstatOutput(containerId, output) {
     ...
     */
    async cleanupNetworkNamespace(containerId) {
        try {
