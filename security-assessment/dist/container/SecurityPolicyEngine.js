...
exports.SecurityPolicyEngine = void 0;
const dockerode_1 = __importDefault(require("dockerode"));
const NetworkMonitor_1 = require("./NetworkMonitor");
const ExternalResourceManager_1 = require("./ExternalResourceManager");
const utils_1 = require("../utils");
const retry_1 = require("../utils/retry");
class SecurityPolicyEngine {
    constructor(networkMonitoringConfig) {
        this.docker = new dockerode_1.default();
        this.networkMonitor = new NetworkMonitor_1.NetworkMonitor(networkMonitoringConfig || NetworkMonitor_1.NetworkMonitor.createDefaultConfig());
        this.externalResourceManager = new ExternalResourceManager_1.ExternalResourceManager(ExternalResourceManager_1.ExternalResourceManager.createDefaultConfig());
     ...
     */
    async applyNetworkIsolation(containerId, config) {
        try {
                    ...
                    for (const networkName of Object.keys(networks)) {
                        try {
                            ...
                            if (this.isSecuritySensitiveNetwork(networkName)) {
                                throw new utils_1.NetworkSecurityViolationError(`Attempt to disconnect from security-sensitive network: ${networkName}`, { containerId, networkName });
                            }
                            const network = this.docker.getNetwork(networkName);
                            await network.disconnect({ Container: containerId, Force: true });
                            utils_1.logger.debug(`Disconnected container ${containerId} from network ${networkName}`);
                        }
                        catch (error) {
                            if (error instanceof utils_1.NetworkSecurityViolationError) {
                                throw error;
                            }
                            // Log warning for non-critical network disconnection failures
                            utils_1.logger.warn(`Failed to disconnect from network ${networkName}`, {
                                containerId,
                                networkName,
                                error: error.message,
                            });
            ...
            }, retry_1.DEFAULT_RETRY_OPTIONS.networkOperation, `applyNetworkIsolation-${containerId}`);
            if (!result.success) {
                throw new utils_1.NetworkSecurityViolationError(`Failed to apply network isolation after ${result.attempts} attempts: ${result.error?.message}`, { containerId, attempts: result.attempts });
            }
            utils_1.logger.info(`Applied network isolation for container ${containerId}`, {
            ...
            if (utils_1.ErrorHandler.isSecurityViolation(assessmentError)) {
                utils_1.logger.error(`Network security violation detected`, {
                    containerId,
            ...
            utils_1.logger.error(`Failed to apply network isolation for container ${containerId}`, {
                error: assessmentError.message,
                code: assessmentError.code,
            });
    ...
    async setResourceLimits(containerId, limits) {
        try {
            ...
            }, retry_1.DEFAULT_RETRY_OPTIONS.resourceAllocation, `setResourceLimits-${containerId}`);
            if (!result.success) {
                // Attempt graceful degradation
                const degradedLimits = this.createDegradedResourceLimits(limits);
                utils_1.logger.warn(`Resource allocation failed, attempting degraded limits`, {
                    containerId,
                    originalLimits: limits,
                    degradedLimits,
                });
                // Try with reduced resources
                const degradedResult = await (0, retry_1.withRetry)(async () => {
                    const container = this.docker.getContainer(containerId);
                    ...
                    return { memoryBytes, cpuQuota };
                }, { ...retry_1.DEFAULT_RETRY_OPTIONS.resourceAllocation, maxAttempts: 2 }, `setResourceLimits-degraded-${containerId}`);
                if (!degradedResult.success) {
                    throw new utils_1.ResourceAllocationError(`Failed to set resource limits even with degraded values after ${result.attempts + degradedResult.attempts} total attempts`, { containerId, originalLimits: limits, degradedLimits });
                }
                utils_1.logger.info(`Applied degraded resource limits for container ${containerId}`, {
                ...
                utils_1.logger.info(`Applied resource limits for container ${containerId}`, {
            ...
            utils_1.logger.error(`Failed to set resource limits for container ${containerId}`, {
                error: assessmentError.message,
                code: assessmentError.code,
                limits,
            });
     ...
     */
    async configureFilesystemAccess(containerId, permissions) {
        try {
            const container = this.docker.getContainer(containerId);
     ...
     */
    async enableSecurityMonitoring(containerId) {
        try {
     ...
     */
    async disableSecurityMonitoring(containerId) {
        try {
     ...
     */
    async createIsolatedNetworkNamespace(containerId) {
        try {
     ...
     */
    async setupControlledProxyAccess(containerId, proxyHost, proxyPort, allowedDomains) {
        try {
     ...
     */
    async downloadExternalResource(containerId, resourceName, version, expectedChecksum) {
        try {
            return await this.externalResourceManager.downloadDependency(containerId, resourceName, version, expectedChecksum);
        }
        catch (error) {
     ...
     */
    async createRestrictedNetwork(containerId, config) {
     ...
     */
    async setMountPermissions(containerId, mountPath, readOnly) {
        const container = this.docker.getContainer(containerId);
        try {
     ...
     */
    async applyRootfsRestrictions(containerId) {
        const container = this.docker.getContainer(containerId);
        try {
     ...
     */
    setupNetworkMonitoringEvents() {
        this.networkMonitor.on('suspiciousActivity', (activity) => {
            utils_1.logger.warn('Suspicious network activity detected', { activity });
        ...
        this.networkMonitor.on('alertThresholdExceeded', (alert) => {
            utils_1.logger.warn('Network alert threshold exceeded', { alert });
    ...
    validateNetworkConfig(config) {
        if (config.allowedHosts && config.allowedHosts.length > 100) {
            throw new utils_1.NetworkSecurityViolationError('Too many allowed hosts specified (maximum 100)', { allowedHostsCount: config.allowedHosts.length });
        }
        // Validate allowed hosts format
        if (config.allowedHosts) {
            for (const host of config.allowedHosts) {
                if (!this.isValidHostname(host)) {
                    throw new utils_1.NetworkSecurityViolationError(`Invalid hostname format: ${host}`, { invalidHost: host });
                }
            }
        }
    }
     ...
     */
    isSecuritySensitiveNetwork(networkName) {
    ...
    validateResourceLimits(limits) {
        ...
        if (memoryBytes < 64 * 1024 * 1024) { // Minimum 64MB
            throw new utils_1.ResourceAllocationError('Memory limit too low (minimum 64MB)', { memoryLimit: limits.memory });
        }
        if (memoryBytes > 8 * 1024 * 1024 * 1024) { // Maximum 8GB
            throw new utils_1.ResourceAllocationError('Memory limit too high (maximum 8GB)', { memoryLimit: limits.memory });
        }
        // Validate CPU limit
        const cpuFloat = parseFloat(limits.cpu);
        if (cpuFloat < 0.1) {
            throw new utils_1.ResourceAllocationError('CPU limit too low (minimum 0.1)', { cpuLimit: limits.cpu });
        }
        if (cpuFloat > 4.0) {
            throw new utils_1.ResourceAllocationError('CPU limit too high (maximum 4.0)', { cpuLimit: limits.cpu });
        }
        // Validate disk space
        if (!limits.diskSpace.match(/^\d+[kmg]$/i)) {
            throw new utils_1.ResourceAllocationError('Invalid disk space format (use format like 1g, 512m, etc.)', { diskSpace: limits.diskSpace });
        }
    }
    /**
     * Checks if requested resources are available on the system
     */
    async checkResourceAvailability(memoryBytes, cpuQuota) {
        try {
            // Get system information
            const systemInfo = await this.docker.info();
            // Check memory availability
            if (systemInfo.MemTotal && memoryBytes > systemInfo.MemTotal * 0.8) {
                throw new utils_1.ResourceAllocationError('Requested memory exceeds 80% of system memory', {
            ...
            if (systemInfo.NCPU && requestedCores > systemInfo.NCPU * 0.8) {
                throw new utils_1.ResourceAllocationError('Requested CPU exceeds 80% of system CPU', {
                    requestedCores,
            ...
            if (error instanceof utils_1.ResourceAllocationError) {
                throw error;
            }
            // If we can't check system resources, log a warning but don't fail
            utils_1.logger.warn('Could not check system resource availability', {
                error: error.message,
            });
        }
    }
     ...
     */
    createDegradedResourceLimits(originalLimits) {
        // Reduce memory by 50%
     ...
     */
    async triggerSecurityAlert(containerId, violation) {
        utils_1.logger.error('SECURITY ALERT: Violation detected', {
            containerId,
...
//# sourceMappingURL=SecurityPolicyEngine.js.map